...
bool PutAnswerToClient(char* name, sockaddr* to, int* lto) { //ответ клиенту
...
bool AcceptCycle(int sq) // принимает подкл юзеров
{
	bool rc = false;
	Contact c(Contact::ACCEPT, "EchoServer");
	while (sq-- > 0 && !rc)
	{
		if ((c.s = accept(sS, (sockaddr*)&c.prms, &c.lprms)) == INVALID_SOCKET) //разрешает подкл сокету
		{
...
DWORD WINAPI AcceptServer(LPVOID pPrm)    //Подключение клиентов к серверу.   для передачи адреса области памяти, в котором будет храниться команда управления
{
...
DWORD WINAPI ConsolePipe(LPVOID pPrm)    // Серверная часть консоли управления, получаем команды управления через именованный канал
{
	DWORD rc = 0;
	char rbuf[100];
...
DWORD WINAPI DispatchServer(LPVOID pPrm) //отправка. Диспетчеризация запросов клиента()
{
	DWORD rc = 0;
					...
					if (delta > 10000) {
						...
						else if (!strcmp(i->msg, "Msg"))
						{
							if (!is_load_library)
							{
								...
								if ((send(i->s, "NOTLib", strlen("NOTLib"), NULL)) == SOCKET_ERROR)
									throw  SetErrorMsgText("send:", WSAGetLastError());
